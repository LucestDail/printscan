document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadPrintHistory() {
        try {
            ...
            const response = await fetch(`/api/print/history?page=${currentPage}&size=${pageSize}&department=${department}&search=${search}${dateFilter}`);
            
            if (!response.ok) {
                throw new Error('인쇄 이력을 가져오는데 실패했습니다.');
            }
            
            const data = await response.json();
            renderPrintHistory(data.content);
    ...
    async function loadStatistics() {
        try {
            ...
            const response = await fetch(`/api/print/statistics?department=${department}${dateFilter}`);
    ...
    // 인쇄 이력 목록 렌더링
    function renderPrintHistory(jobs) {
        ...
        if (jobs.length === 0) {
            tbody.innerHTML = `
                <tr>
                    <td colspan="11" class="text-center">인쇄 이력이 없습니다.</td>
                </tr>
            `;
            return;
        }
        ...
        jobs.forEach(job => {
            ...
            row.innerHTML = `
                ...
                <td>${job.requester}</td>
                <td>${job.copies}</td>
                <td>${job.printOptions.color ? '예' : '아니오'}</td>
    ...
    // 재인쇄 요청
    window.resubmitJob = async function(jobId) {
        try {
            if (!confirm('이 작업을 재인쇄하시겠습니까?')) {
                return;
            }
            
            showLoading();
            
            const response = await fetch(`/api/print/jobs/${jobId}/resubmit`, {
                method: 'POST'
            });
            
            if (!response.ok) {
                throw new Error('재인쇄 요청에 실패했습니다.');
            }
            
            showNotification('재인쇄가 요청되었습니다.');
            ...
            // 인쇄 현황 페이지로 이동
            window.location.href = '/pages/print/status.html';

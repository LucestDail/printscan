...
/**
 * 호스트 OS(CUPS)에 등록된 프린터로 javax.print 전송. USB Zebra 를 CUPS raw 큐로 등록하면 동작.
 * 프린터 이름은 PrinterProperties.name(콤마 구분, 소문자 부분일치)로 찾는다.
 */
@Slf4j
@Component
    ...
    public void send(String zpl) throws Exception {
        ...
        javax.print.PrintService printer = Arrays.stream(PrintServiceLookup.lookupPrintServices(null, null))
                ...
                .orElseThrow(() -> new IllegalStateException(
                        "CUPS 에서 프린터를 찾지 못했습니다(name=" + props.getName() + "). lpstat -p 로 확인."));

        DocPrintJob job = printer.createPrintJob();
        Doc doc = new SimpleDoc(zpl.getBytes(StandardCharsets.UTF_8), DocFlavor.BYTE_ARRAY.AUTOSENSE, null);
        job.print(doc, null);
        log.info("[print] cups printer='{}' ({} bytes)", printer.getName(), zpl.length());
    }
}
